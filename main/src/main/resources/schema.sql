DROP TABLE IF EXISTS participation_request;
DROP TABLE IF EXISTS compilation_events;
DROP TABLE IF EXISTS compilations;
DROP TABLE IF EXISTS participant_confirmed_events;
DROP TABLE IF EXISTS events;
DROP TABLE IF EXISTS users_subscriptions;
DROP TABLE IF EXISTS users_event_vision_blacklist;
DROP TABLE IF EXISTS subscriprion_request;
DROP TABLE IF EXISTS users;
DROP TABLE IF EXISTS categories;
DROP TABLE IF EXISTS locations;

CREATE TABLE users(
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    name VARCHAR(250) NOT NULL,
    email VARCHAR(254) NOT NULL UNIQUE,
    subscribers_mode VARCHAR(30),
    created_event_vision_mode VARCHAR(30),
    participation_event_vision_mode VARCHAR(30)
);

CREATE TABLE IF NOT EXISTS users_subscriptions(
    blogger_id BIGINT,
    subscriber_id BIGINT,
    FOREIGN KEY (blogger_id) REFERENCES users(id),
    FOREIGN KEY (subscriber_id) REFERENCES users(id)
);

CREATE TABLE IF NOT EXISTS users_event_vision_blacklist(
    blogger_id BIGINT,
    subscriber_id BIGINT,
    FOREIGN KEY (subscriber_id) REFERENCES users(id),
    FOREIGN KEY (blogger_id) REFERENCES users(id)
);

CREATE TABLE IF NOT EXISTS subscriprion_request(
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    created TIMESTAMP WITHOUT TIME ZONE NOT NULL,
    subscriber_id BIGINT NOT NULL,
    blogger_id BIGINT NOT NULL,
    status VARCHAR(50),
    FOREIGN KEY (subscriber_id) REFERENCES users(id),
    FOREIGN KEY (blogger_id) REFERENCES users(id)
    );

CREATE TABLE IF NOT EXISTS categories(
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    name VARCHAR(50) NOT NULL UNIQUE
);

CREATE TABLE locations(
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    lat FLOAT NOT NULL,
    lon FLOAT NOT NULL
);

CREATE TABLE IF NOT EXISTS compilations(
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    pinned BOOLEAN NOT NULL,
    title VARCHAR(50) NOT NULL UNIQUE
);

CREATE TABLE IF NOT EXISTS events(
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    annotation VARCHAR(2000) NOT NULL UNIQUE,
    description VARCHAR(7000),
    title VARCHAR(120) NOT NULL,
    state VARCHAR(10),
    participants_limit INTEGER,
    confirmed_requests INTEGER,
    is_paid BOOLEAN NOT NULL,
    category_id BIGINT NOT NULL,
    initiator_id BIGINT NOT NULL,
    locations_id BIGINT NOT NULL,
    creation_date TIMESTAMP WITHOUT TIME ZONE,
    event_date TIMESTAMP WITHOUT TIME ZONE NOT NULL,
    published_on TIMESTAMP WITHOUT TIME ZONE,
    request_moderation BOOLEAN,
    views BIGINT,
    FOREIGN KEY (category_id) REFERENCES categories(id),
    FOREIGN KEY (initiator_id) REFERENCES users(id),
    FOREIGN KEY (locations_id) REFERENCES locations(id)
);

CREATE TABLE IF NOT EXISTS participant_confirmed_events(
    participant_id BIGINT,
    event_id BIGINT,
    FOREIGN KEY (event_id) REFERENCES events(id),
    FOREIGN KEY (participant_id) REFERENCES users(id)
);

CREATE TABLE IF NOT EXISTS participation_request(
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    created TIMESTAMP WITHOUT TIME ZONE NOT NULL,
    event_id BIGINT NOT NULL,
    requester_id BIGINT NOT NULL,
    status VARCHAR(50),
    FOREIGN KEY (event_id) REFERENCES events(id),
    FOREIGN KEY (requester_id) REFERENCES users(id)
);

CREATE TABLE IF NOT EXISTS compilations_events(
    compilation_id BIGINT NOT NULL,
    event_id BIGINT NOT NULL,
    PRIMARY KEY (compilation_id, event_id),
    FOREIGN KEY (compilation_id) REFERENCES compilations(id),
    FOREIGN KEY (event_id) REFERENCES events(id)
);